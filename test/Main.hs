...
testFwa01 :: Assertion
testFwa01 = sequence_ $ fmap (\name -> assertFwa "Cannot parse" name (const True)) testFiles

testFwa02 :: Assertion
testFwa02 = assertFwa "Incorrect state count" (head testFiles) (\fwa -> length (Fwa.states fwa) == 4)

testFwa03 :: Assertion
testFwa03 = assertFwa "Incorrect transition count" (head testFiles) (\fwa -> length (Fwa.transitions fwa) == 14)
...
testFta02 :: Assertion
testFta02 = assertFta "Incorrect state count" (head testFiles) (\fta -> length (Fta.states fta) == 4)

testFta03 :: Assertion
testFta03 = assertFta "Incorrect transition count" (head testFiles) (\fta -> length (Fta.transitions fta) == 14)
