...
labelList = sepEndByToSet labelDecl (char ' ')

labelDecl :: Parser (Label, Rank)
labelDecl = do { label <- Parsing.Fta.label
               ; char ':'
               ; rank <- decimal
               ; return (label, rank)
               }
...
stateList = sepEndByToSet state (char ' ')

state :: Parser State
state = many1 alphaNum

transitionList :: Parser (Set Transition)
transitionList = sepEndByToSet transition endOfLine

transition :: Parser Transition
transition = do { label <- Parsing.Fta.label
...
transitionStateList = option empty (brackets (sepEndByToSet state (char ',')))

label :: Parser Label
label = many1 alphaNum

sepEndByToSet :: (Ord a, Stream s m t) => ParsecT s u m a -> ParsecT s u m sep -> ParsecT s u m (Set a)
sepEndByToSet value separator = fmap fromList (value `sepEndBy` separator)
