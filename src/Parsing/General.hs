...
loadFwa :: (Monad m) => FilePath -> IO (m (Fwa Fwa.State))
loadFwa filePath =
  fmap parseFwa (B.readFile filePath)

parseFwa :: (Monad m) => B.ByteString -> m (Fwa Fwa.State)
...
ftaToFwa :: Fta -> Maybe (Fwa Fwa.State)
...
ftaToFwaTransition :: Fta.Transition -> Fwa.Transition Fwa.State
